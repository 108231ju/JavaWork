...
SELECT HEIGHT FROM T_STUDENT WHERE HEIGHT >= 180;

--2.  t_student 테이블에서 몸무게(weight)가 60~80 인 사람의 이름과 체중 출력
SELECT WEIGHT FROM T_STUDENT WHERE WEIGHT >= 60 AND WEIGHT <= 80;

--3.  t_student 테이블에서는 제1전공(deptno1) 이 101번 학과학생과 201번 학과 학생들을 모두 출력하세요 (IN 연산자 활용)
SELECT DEPTNO1 FROM T_STUDENT WHERE DEPTNO1 IN (101,201);
