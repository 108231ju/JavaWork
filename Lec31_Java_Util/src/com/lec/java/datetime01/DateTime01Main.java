...
public class DateTime01Main {
	...
	public static void main(String[] args) throws Exception{
		...
		int month = now.getMonth() + 1;	//0이 1일
		int date = now.getDate();	//날짜
		int day = now.getDay(); //Day 는 요일이 숫자로 나옴 0 --> 일
		int hour = now.getHours();
		int minute = now.getMinutes();
		int second = now.getSeconds();
		System.out.println(year + "년" + month + "월" + date + "일" + day + "요일" + hour +"시"+ minute +"분"
				+ second + "초");
		...
		long baseTime = System.currentTimeMillis();	// 현재시각 ms 로 리턴
										// 1970-01-01 00:00:00 UTC 기준으로 경과된ms
		...
		long durationInMills = 1000003;
		long millis = durationInMills % 1000;
		long sec = (durationInMills / 1000) % 60;
		long min = (durationInMills / (1000 * 60)) % 60;
		long hr = (durationInMills / (1000 * 60 * 60)) % 24;
